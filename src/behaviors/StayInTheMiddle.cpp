...
void StayInTheMiddle::stay_in_the_middle(const pteam_p2os::Perception in, float threashold, int threashold_valley) {
  ...
  for(int i = 0; i < in.laser.data.scan_msg.ranges.size(); i++) {
    if(isnan(in.laser.data.scan_msg.ranges[i])) {
      //se trovo un valore nan setto momentaneamente il valore nel polar histogram a 0
      find_nan = true;
      polar_histogram[i] = 0;
    }
    else {
      polar_histogram[i] = in.laser.data.scan_msg.range_max - in.laser.data.scan_msg.ranges[i];
      
      if(find_nan) {
	//calcolo il valore medio da assegnare ai settori nan
	avg_value = (last-value - polar_histogram[i]) / 2;	//TODO: media pesata in base all'indice
  ...
  for(int i = 0; i < polar_histogram.size(); i++) {
